...
│   └── aiAgentApi.js     # ai-agent 服务接口（单轮对话 + SSE 流式对话 + 会话列表管理）
...
| AI 助手 | `/ai-assistant` → `AiAssistantPage.vue` | 对接 ai-agent 服务：单轮对话框 + SSE 流式渲染回答（`fetch` + `ReadableStream` 手动解析 `event:`/`data:` 帧，因为响应用了具名 event 且需要携带 Authorization 头，浏览器原生 EventSource 均不满足）；多轮会话记忆（`session_id`，首轮传空由后端生成，续聊原样带上，存组件内存不落路由避免打断流式渲染）；左侧会话列表侧边栏，支持新建对话、切换会话（点击自动拉取该会话完整历史消息回显，非仅接续对话）、重命名（弹窗）、置顶/取消置顶、归档/取消归档（可切换显示已归档）、删除（二次确认）；助手回复用 `AxMarkdown` 渲染（标题/列表/代码块高亮/复制按钮），用户消息保持纯文本；历史消息里的工具调用过程消息（`role: tool`、空内容的 assistant 过程消息）不渲染成气泡，只回显最终问答；用户消息与助手消息气泡区分展示，生成中显示打字光标动效，支持「停止生成」（仅中止前端接收，已生成内容保留）与「清空对话」（等同新建对话）；SSE 请求过期（access token 15 分钟）后自动续期重试；租户身份（`X-Tenant-Code`）由后端网关 ForwardAuth 按登录用户自动注入，前端调用无需关心，未开户/无租户账号会收到 400 提示 |
...
- [ai-agent](https://github.com/JIAOZAI1/lead-mind-ai-agent)：`POST /ai-agent/v1/chat` 单轮对话（非流式，请求体带 `session_id` 续接多轮历史，首轮传空）；`GET /ai-agent/v1/chat/stream?message=&session_id=` SSE 流式对话，响应用具名 event（`session`/`message`/`done`/`error`），首帧 `event: session` 回显本轮生效的 `session_id`，`message` 帧 `data` 里的 `delta` 为增量文本，`done` 事件即结束标志（无累计全文字段，前端自行拼接）；`session_id` 为空或首次出现时后端新建会话并在 Redis 落地短期历史（受 compaction 影响，会被摘要/截断），非空时按该 ID 续接上下文；`GET /ai-agent/v1/sessions?include_archived=`、`PATCH /ai-agent/v1/sessions/{id}`（改标题/置顶/归档）、`DELETE /ai-agent/v1/sessions/{id}` 管理会话元数据（仅当前登录用户自己的会话，按 `X-User-Id` 过滤）；`GET /ai-agent/v1/sessions/{id}/messages` 拉取会话完整历史消息（不过期的 MySQL 存档，压缩前的原始内容，与驱动模型的短期记忆是两套独立存储），响应数组每项含 `role`（`user`/`assistant`/`tool`）/`content`/`tool_calls`/`tool_call_id`/`tool_name`/`created_at`，会话被删除后该接口对其返回 404（存档数据物理保留但 API 不可达）；服务端按 `X-Tenant-Code` 请求头路由到租户，该头由网关 ForwardAuth 校验登录态时按用户的 active 租户自动注入（管理员等无租户账号会收到 `400 missing X-Tenant-Code header`）；无工具调用中间步骤展示（后端尚未区分 tool-call 与最终回答 chunk）
...
- **2026-07-16**
  ...
  - 开户流程改用 axis-ui 的 `AxWizardModal` 向导组件，分为确认用户、开户设置、执行进度三步；开户设置中新增 License 到期时间，随 `approveReview` 请求体提交 `licenseExpiresAt`，前端校验必须晚于当前时间；开户记录表格新增 License 到期时间列
- **2026-07-17**
...
- **2026-07-25**
  - axis-ui 升级至 0.8.0：新增 `AxMarkdown` 组件（`content`/`copyable`/`breaks` props，`copy` 事件），底层 `marked`（GFM）解析 + `highlight.js` 按需语言包高亮，围栏代码块拆分为独立子块（语言标签 + 复制按钮），配色为组件级 Token（`--ax-markdown-code-*`）随暗色模式自动重映射；`breaks` 默认 `true`，单换行渲染为 `<br>`，官方文档明确标注"适合逐字流式输出的 AI 对话场景"
  - AI 助手页面接入 `AxMarkdown`：助手消息改用 `<ax-markdown :content="msg.content" />` 渲染模型输出（标题、列表、代码块语法高亮、代码复制按钮等），此前是纯文本 `<span>` + `white-space: pre-wrap`；用户消息保持纯文本展示（本来就不是 Markdown 内容，无需过渡解析）；打字光标沿用原有 `.ai-assistant__cursor` 样式，作为紧跟 `AxMarkdown` 渲染内容后的兄弟节点
  - AI 助手接入会话列表侧边栏，对接 ai-agent 新增的会话管理接口：`aiAgentApi.js` 新增 `listSessions()`/`patchSession()`/`deleteSession()`（`GET`/`PATCH`/`DELETE /ai-agent/v1/sessions`）；侧边栏支持新建对话、会话列表（置顶优先+最近活跃排序，可选显示已归档）、切换会话、重命名（弹窗 + `AxInput`）、置顶/取消置顶、归档/取消归档、删除（二次确认弹窗）
  - 归档/删除当前正在查看的会话时自动回到"新建对话"状态（清空消息区与 `sessionId`），避免残留在一个已不可见/已不存在的会话上
- **2026-07-26**
